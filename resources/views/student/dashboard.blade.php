        ...
        <div class="col-md-12">
            <div class="card"> <img class="card-img-top" src="{{asset('assets/images/pro-timeline.jpg')}}" alt="Card image cap">
                <div class="card-body little-profile text-center">
                    @if ($prof_pic !== NULL)
                        <div class="pro-img"><img src="/documents/{{$prof_pic}}" alt="user"></div>
                    @else
                        <div class="pro-img"><img src="{{asset('assets/images/dp.png')}}" alt="user"></div>
                    @endif
                    <h2 class="m-b-0">{{$student_name}} {{$student_mname}}. {{$student_lname}}</h2>
                     <!--</p> <a href="javascript:void(0)" class="m-t-10 waves-effect waves-dark btn btn-primary btn-md btn-rounded" data-abc="true">Follow</a> &nbsp; -->
                     <a href="{{ url('/student/my-pdf') }}" class="m-t-10 waves-effect waves-dark btn btn-success btn-md btn-rounded" data-abc="true">Print Resume</a>
              ...
              <div class="card card-success">
                <div class="card-header">
                  ...
                  <div class="card-tools">
                    <button type="button" class="btn btn-default btn-xs" id="addEduc">
                        Add new
                    </button>
                    ...
                    <div class="col-md-12">
                        @if (isset($educations))
                            ...
                            <div class="callout callout-success">
                               ...
                               @switch($row->educ_level)
                                    @case("College")
                                        <p>Institution Address :<b>{{$row->institution_address}}</b> <br> Education Level: <b>{{$row->educ_level}} </b> <br> Course: <b>{{$row->degree}}</b> <br> From: <b>
                                             {{ date('M d, Y', strtotime($row->from)) }}</b> <br> To: <b>{{ date('M d, Y', strtotime($row->to)) }}</b> </p>
                                        @break
                                    @case("Senior High")
                                        <p>Institution Address : <b>{{$row->institution_address}}</b> <br> Education Level: <b>{{$row->educ_level}} </b> <br> Strand/Track: <b>{{$row->degree}}</b> <br> From: <b>
                                            {{ date('M d, Y', strtotime($row->from)) }}</b> <br> To: <b>{{ date('M d, Y', strtotime($row->to)) }}</b> </p>
                                        @break
                                    @case("Vocational")
                                        <p>Institution Address : <b>{{$row->institution_address}}</b> <br> Education Level: <b>{{$row->educ_level}} </b> <br> Course: <b>{{$row->degree}}</b> <br> From: <b>
                                        ...
                                        @break
                                    @default
                                        <p>Institution Address : <b>{{$row->institution_address}} </b> <br>  Education Level: <b>{{$row->educ_level}}</b> <br> From: <b>
                                            {{ date('M d, Y', strtotime($row->from)) }}</b> <br> To: <b>{{ date('M d, Y', strtotime($row->to)) }}</b> </p>
                                @endswitch
                                <br>
                                <div class="row">
                                    <button class="btn btn-xs btn-info btnEditEduc" data-value="{{$row->id}}">Edit</button>&nbsp;&nbsp;
                                    <button class="btn btn-xs btn-danger btnDeleteEduc" data-value="{{$row->id}}">Remove</button>
                                    <p class="mx-3">
                                        <input type="radio" name="educ_privacy_type" class="toggleBtnEduc" {{ $row->privacy_type == 2 ? 'checked' : '' }} value="2" data-value="{{$row->id}}"> Private
                                        &nbsp;
                                        <input type="radio" name="educ_privacy_type" class="toggleBtnEduc" {{ $row->privacy_type == 1 ? 'checked' : '' }} value="1" data-value="{{$row->id}}"> Public
                                    </p>
                                </div>
                            </div>
                            @endforeach
            ...
            <div class="col-md-12">
                <div class="card card-success">
                  <div class="card-header">
                    ...
                    <div class="card-tools">
                      <button type="button" id="addExperience" class="btn btn-default btn-xs">
                          Add new
                      </button>
                      <button type="button" class="btn btn-tool" data-card-widget="collapse">
                        <i class="fas fa-minus"></i>
                          ...
                          @if (isset($experiences))
                              @foreach ($experiences as $row)
                                <div class="callout callout-success">
                                    ...
                                    <br>
                                    <div class="row">
                                        <button class="btn btn-xs btn-info btnEditExperience" data-value="{{$row->id}}">Edit</button>&nbsp;&nbsp;
                                        <button class="btn btn-xs btn-danger btnDelExperience" data-value="{{$row->id}}">Remove</button>
                                        <p class="mx-3">
                                            <input type="radio" name="exp_privacy_status" class="toggleBtnExp" {{ $row->privacy_status == 2 ? 'checked' : '' }} value="2" data-value="{{$row->id}}"> Private
                                            &nbsp;
                                            <input type="radio" name="exp_privacy_status" class="toggleBtnExp" {{ $row->privacy_status == 1 ? 'checked' : '' }} value="1" data-value="{{$row->id}}"> Public
                                        </p>
                                    </div>
                                </div>
                              @endforeach
            ...
            <div class="col-md-12">
                <div class="card card-success">
                    <div class="card-header">
                        ...
                        <div class="card-tools">
                        @if (!isset($skills[0]))
                            <button type="button" id="addSkills" class="btn btn-default btn-xs">
                                Add new
                            </button>
                        @endif
                        <button type="button" class="btn btn-tool" data-card-widget="collapse">
                            ...
                            <div class="col-md-12">
                                @if (isset($skills[0]))
                                    <div class="callout callout-success">
                                        ...
                                        <br>
                                        <div class="row">
                                            <button class="btn btn-xs btn-info btn-edit-skills" data-value="{{ isset($skills[0]) ? $skills[0]->id : ''}}">Edit</button>&nbsp;&nbsp;
                                            <button class="btn btn-xs btn-danger btn-delete-skills" data-value="{{ isset($skills[0]) ? $skills[0]->id : ''}}">Remove</button>
                                        </div>
                                    </div>
                                @endif
                            </div>
            ...
            <div class="col-md-12">
                <div class="card card-success">
                    <div class="card-header">
                        ...
                        <div class="card-tools">
                        <button type="button" id="addAwardsAchievements" class="btn btn-default btn-xs">
                            Add new
                        </button>
                        <button type="button" class="btn btn-tool" data-card-widget="collapse">
                            <i class="fas fa-minus"></i>
                                 ...
                                 @foreach ($awardsachievement as $row)
                                    ...
                                    <div class="callout callout-success">
                                        ...
                                        <p> Description : <b>{{$row->description}}</b> <br>
                                            ...
                                            Attachment: <b>   <a href ="/documents/{{$row->attachment_filename}}" download> {{$row->attachment_filename}} </a>  <br>
                                            </b> </p>
                                            {{-- <button type="button">@if ($row->privacy_status == 2) Public @elseif($row->privacy_status == 1) Private @endif</button> --}}
                                        <br>
                                        <div class="row">
                                            <button class="btn btn-xs btn-info btnEditAwardsAchievements" data-value="{{$row->id}}">Edit</button>&nbsp;&nbsp;
                                            <button class="btn btn-xs btn-danger btnDelAwardsAchievements" data-value="{{$row->id}}">Remove</button>
                                            <p class="mx-3">
                                                <input type="radio" name="award_privacy_status" class="toggleBtnAward" {{ $row->privacy_status == 2 ? 'checked' : '' }} value="2" data-value="{{$row->id}}"> Private
                                                &nbsp;
                                                <input type="radio" name="award_privacy_status" class="toggleBtnAward" {{ $row->privacy_status == 1 ? 'checked' : '' }} value="1" data-value="{{$row->id}}"> Public
                                            </p>
                                        </div>
                                    </div>
                                    @endforeach
            ...
            <div class="col-md-12">
                <div class="card card-success">
                    <div class="card-header">
                        ...
                        <div class="card-tools">
                        <button type="button" id="addCertification"  class="btn btn-default btn-xs">
                            Add new
                        </button>
                        <button type="button" class="btn btn-tool" data-card-widget="collapse">
                            <i class="fas fa-minus"></i>
                        ...
                        <div class="row">
                            ...
                            @if (isset($certification))
                                ...
                                <div class="callout callout-success">
                                    ...
                                    <p> Certification Author: &nbsp; <b>{{$row->cert_author}}</b> 
                                    <br> Certification Address : &nbsp;<b>{{$row->cert_address}} </b> <br> 
                                    Type:<b>{{$row->type}}</b> <br> Date Given: <b>{{ date('M d, Y', strtotime($row->date_give))}} </b> 
                                    <br> Expiration: &nbsp; <b> {{ date('M d, Y', strtotime($row->expiration))}}</b> 
                                    <br> Attachment : <b> <a href ="/documents/{{$row->attachment_file}}" download> {{$row->attachment_file}} </a>  <br>
                                            </b>
                                    
                                    </p>
                                    <br>
                                    <div class="row">
                                        <button class="btn btn-xs btn-info btnEditCertification" data-value="{{$row->id}}">Edit</button>&nbsp;&nbsp;
                                        <button class="btn btn-xs btn-danger btnDelCertification" data-value="{{$row->id}}">Remove</button>
                                        <p class="mx-3">
                                            <input type="radio" name="exp_privacy_status" class="toggleBtnExp" {{ $row->privacy_status == 2 ? 'checked' : '' }} value="2" data-value="{{$row->id}}"> Private
                                            &nbsp;
                                            <input type="radio" name="exp_privacy_status" class="toggleBtnExp" {{ $row->privacy_status == 1 ? 'checked' : '' }} value="1" data-value="{{$row->id}}"> Public
            ...
            <div class="col-md-12">
                <div class="card card-success">
                    <div class="card-header">
                        ...
                        <div class="card-tools">
                        <button type="button" id="addProject" class="btn btn-default btn-xs">
                            Add new
                        </button>
                        <button type="button" class="btn btn-tool" data-card-widget="collapse">
                            <i class="fas fa-minus"></i>
                            ...
                            <div class="col-md-12">
                                @if (isset($project))
                                    ...
                                    <div class="callout callout-success">
                                        ...
                                        <p> Position: &nbsp; <b>{{$row->position}}</b> <br>  Description: &nbsp; <b>{{$row->project_description}}</b><br>Date_established: &nbsp; <b>{{ date('M d, Y ', strtotime($row->date_established))}}</b> <br> Url: &nbsp; <b>{{$row->url}}</b> <br> File Upload: &nbsp; <b><a href ="/documents/{{$row->file_upload}}" download> {{$row->file_upload}} </a></b> </p>
                                        <br>
                                        <div class="row">
                                            <button class="btn btn-xs btn-info btnEditProjects" data-value="{{$row->id}}">Edit</button>&nbsp;&nbsp;
                                            <button class="btn btn-xs btn-danger btnDelProjects" data-value="{{$row->id}}">Remove</button>
            ...
            <div class="col-md-12">
                <div class="card card-success">
                    <div class="card-header">
                        ...
                        <div class="card-tools">
                        <button type="button" id="addOrganization" class="btn btn-default btn-xs">
                            Add new
                        </button>
                        <button type="button" class="btn btn-tool" data-card-widget="collapse">
                            <i class="fas fa-minus"></i>
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                @if (isset($org))
                                    ...
                                    <br>
                                    <div class="row">
                                        <button class="btn btn-xs btn-info btnEditOrg" data-value="{{$row->id}}">Edit</button>&nbsp;&nbsp;
                                            <button class="btn btn-xs btn-danger btnDelOrg" data-value="{{$row->id}}">Remove</button>
                                    </div>
                                </div>
                                @endforeach
                                @endif
            ...
            <div class="col-md-12">
                <div class="card card-success">
                    <div class="card-header">
                        ...
                        <div class="card-tools">
                            <button id="addSeminar" type="button" class="btn btn-default btn-xs">
                            Add new
                            </button>
                            <button type="button" class="btn btn-tool" data-card-widget="collapse">
                            <i class="fas fa-minus"></i>
                            ...
                            <div class="col-md-12">
                                ...
                                @foreach ($seminar as $row)
                                  <div class="callout callout-success">
                                      ...
                                      <p> Venue: &nbsp; <b>{{$row->venue}}</b><br>  Type: &nbsp; <b>{{$row->type}}</b> <br> Date Given: &nbsp; <b>{{ date('M d, Y ', strtotime($row->date_given))}}</b> <br> Attachment: &nbsp; <b><a href ="/documents/{{$row->attachment_filename}}" download> {{$row->attachment_filename}} </a></b> </p>
                                      <br>
                                      <div class="row">
                                          <button class="btn btn-xs btn-info btnEditSeminar" data-value="{{$row->id}}">Edit</button>&nbsp;&nbsp;
                                          <button class="btn btn-xs btn-danger btnDelSeminar" data-value="{{$row->id}}">Remove</button>
        ...
        <form action="{{ url('student/save-educ') }}" method="POST">
            ...
            <div class="modal-body">
                <div class="col-md-12">
                    ...
                    <div class="form-group" id="vocational" hidden>
                        <label for="">Course</label>
        ...
        <form action="{{ url('student/save-certification') }}" method="POST" enctype="multipart/form-data">
          ...
          <div class="modal-body">
              ...
              <div class="col-md-12">
                  ...
                  <div class="form-group">
                    <label for="">Date Given</label>
                    <input type="date" id="date" class="form-control" name="date" value="2022-01-18" placeholder="Enter Start Date">
                  </div>
                  <div class="form-group">
                    <label for="">Attachment Filename</label>
                    <input type="file" class="form-control" name="attachment_filename" id="attachment_filename">
...
@section('user-js')
    ...
    <script>
        $(document).ready(function(){
            $(".toggleBtnEduc").on('change', function(){
                // console.log($(this).val());
                $.ajax({
                    url: "{{ url('/student/edit-educ/change-privacy') }}",
                    type: "POST",
                    data: {i:$(this).attr('data-value'), v:$(this).val(), '_token':$('meta[name="csrf-token"]').attr('content')},
            ...
            $(".toggleBtnExp").on('change', function(){
                // console.log($(this).val());
                $.ajax({
                    url: "{{ url('/student/edit-experience/change-privacy') }}",
                    type: "POST",
                    data: {i:$(this).attr('data-value'), v:$(this).val(), '_token':$('meta[name="csrf-token"]').attr('content')},
                    success: function (response)
                    {
            ...
            $(".toggleBtnCer").on('change', function(){
                // console.log($(this).val());
                $.ajax({
                    url: "{{ url('/student/edit-certification/change-privacy') }}",
                    type: "POST",
                    data: {i:$(this).attr('data-value'), v:$(this).val(), '_token':$('meta[name="csrf-token"]').attr('content')},
                    success: function (response)
                    {
            ...
            $(".toggleBtnAward").on('change', function(){
                // console.log($(this).val());
                $.ajax({
                    url: "{{ url('/student/edit-awardsachievement/change-privacy') }}",
                    type: "POST",
                    data: {i:$(this).attr('data-value'), v:$(this).val(), '_token':$('meta[name="csrf-token"]').attr('content')},
                    success: function (response)
                    {
            ...
            $("#level").on("change",function(){
                if($(this).val() == "Vocational") {
                    $("#vocational").removeAttr("hidden","hidden");
                } else {
                    $("#vocational").attr("hidden","hidden");
                }
            });
                ...
                $.ajax({
                    ...
                    success: function (response)
                    {
                        if (response) {
                            ...
                            $("#vocational").val(response[0].degree != null ? response[0].degree : '');
